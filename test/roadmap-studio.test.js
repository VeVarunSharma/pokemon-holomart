...
test("loads the workspace artifact and rejects paths outside the JSON boundary", async () => {
  ...
  const opened = await prepareRoadmapOpen(projectWorkspaceFromExtension(extensionUrl), {
    initiativeFocus: "saved-views",
  });
  assert.equal(opened.roadmapPath, "product/roadmap.json");
  assert.equal(opened.focus, "init-saved-views-preview");
});
...
test("filters by horizon and status without mutating the roadmap", () => {
  ...
  assert.deepEqual(filtered.map((item) => item.id), ["init-personal-view-sync"]);
  assert.deepEqual(roadmap, original);
  assert.throws(() => filterInitiatives(roadmap, { horizon: "someday" }), /Unknown horizon/);
});

test("focus validates ids and the renderer makes Saved Views central", () => {
  const focused = focusInitiative(roadmap, "init-saved-views-preview");
  assert.match(focused.title, /Saved Views/);
  assert.equal(resolveInitiativeFocus(roadmap, "saved-views").id, focused.id);
  ...
  assert.match(html, /initiative focused saved-views/);
...
test("render focus distinguishes an absent query parameter from an explicit clear", () => {
  const focusedId = "init-saved-views-preview";
...
test("handoff is evidence-grounded and covers engineering decision inputs", () => {
  const handoff = draftHandoff(roadmap, "init-saved-views-preview");
  ...
  assert.match(handoff, /risk-silent-stale-state/);
  assert.match(handoff, /## Success measures/);
  assert.match(handoff, /## Human decisions/);
  assert.match(handoff, /decision-duplicate-names/);
  assert.match(handoff, /work-view-storage-validation/);
});
