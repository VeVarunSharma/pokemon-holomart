    ...
    run: |
      ...
      function loadPlaywright() {
        try {
          ...
          // The playwright-cli global install nests the "playwright" package under its
          // own node_modules instead of hoisting it to the global root, so a plain
          // require("playwright") fails even though the browsers are installed.
          try {
            const globalRoot = execFileSync("npm", ["root", "--global"], { encoding: "utf8" }).trim();
            return require(path.join(globalRoot, "@playwright", "cli", "node_modules", "playwright"));
          } catch {
            throw primaryError;
          }
        }
